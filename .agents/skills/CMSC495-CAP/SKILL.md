...
An employee asks a question, the backend retrieves passages from MongoDB Atlas
...
| Layer | What | Where |
|---|---|---|
| API | FastAPI, uvicorn, slowapi rate limiting, JWT via python-jose, passlib+bcrypt | `backend/` |
| RAG pipeline | OpenAI embeddings and chat, LangChain text splitters, pymongo | `src/` |
| Storage | MongoDB Atlas (passages, conversations, escalations, caches, query logs), S3 for raw documents | `src/mongo.py`, `backend/db.py` |
| Frontend | React 19, TypeScript (strict, no unused locals), Vite, Tailwind 4, react-router, axios | `frontend/` |
| Serving | Docker Compose, Nginx in front of the API, EC2 | `docker-compose.yml`, `frontend/nginx.conf`, `scripts/` |
| Tests | pytest with everything external stubbed; ~100 tests, about a second | `tests/` |
| Lint | ruff (Python), ESLint flat config + tsc (frontend) | `ruff.toml`, `frontend/eslint.config.js` |
...
```text
backend/            FastAPI app. main.py mounts routes/; db.py binds collection handles at import
...
src/                the pipeline, imported by the backend by bare module name
...
`backend/` and `src/` are both put on `sys.path`, so modules import each other
as `from config import ...`, not `from src.config`. Run backend code from
`backend/`, or through the stub, tests, or Docker. `python backend/main.py`
from the repo root fails to find `config`.

## Set up and run

```bash
make setup        # .venv, Python deps (all three requirements files), npm install
...
Real services need `.env` from `.env.example`, then `src/seed_documents.py`,
`src/embed_documents.py`, and a vector index created by hand in the Atlas UI.
...
Vite build, both Docker images built and the backend one import-smoked,
...
- `ruff.toml` is the rule set: pycodestyle, pyflakes, isort, bugbear,
...
- New tuning knobs go in `src/config.py` as `NAME = type(os.getenv("NAME", default))`.
- Vendor-specific code goes in `src/llm.py` only. A new provider is a subclass
  registered in `_PROVIDERS` and selected with `LLM_PROVIDER`.
- No test-only branches in `src/` or `backend/`. If something cannot be
...
- Components live under `frontend/src/components/<Area>/`, hooks in `hooks/`
  with a `use` prefix, API calls through `api/client.ts`.
- `frontend/src/config.ts` mirrors `src/config.py` for `APP_NAME` and
...
| Change | File |
|---|---|
| a tuning knob | `src/config.py` |
| the answer prompt | `src/rag_chain.py` `ANSWER_SYSTEM_PROMPT`, then `PROMPT_VERSION` |
| retrieval or the grounding gate | `src/rag_chain.py` |
| a model vendor | `src/llm.py` |
| how a source format is parsed | `src/documents.py` |
| an API endpoint | `backend/routes/<area>.py`, mounted in `backend/main.py` |
| a collection or index | `backend/db.py` |
| the chat UI | `frontend/src/components/Chat/`, state in `hooks/useChat.ts` |
| the sample corpus | `data/sample-policies/`, then re-run ingestion |
| CI behaviour | `.github/workflows/ci.yml`; lint rules in `ruff.toml` |
