    ...
    for source, title, category, count in docs:
        for i in range(count):
            FAKE_DB["passages"].insert_one({
                "source": source, "doc_id": source.split("/")[-1][:-3], "title": title,
                "category": category, "owner": None, "effective_date": None,
                "chunk_index": i,
...
def test_library_is_built_from_passages_without_vectors(client, auth):
    _seed_passages(("documents/pto.md", "PTO", "Leave", 3), ("documents/expenses.md", "Expenses", "Finance", 2))
...
def test_search_and_filters_and_paging(client, auth):
    ...
    assert client.get("/api/documents", params={"category": "Two"}, headers=auth).json()["items"][0]["title"] == "Beta"
...
def test_passages_are_returned_in_order(client, auth):
    ...
    texts = client.get("/api/documents/passages", params={"source": "documents/pto.md"}, headers=auth).json()
    assert [t.split("\n")[1][:11] for t in texts] == ["Paragraph 0", "Paragraph 1", "Paragraph 2"]
    assert client.get("/api/documents/passages", params={"source": "nope"}, headers=auth).status_code == 404
...
def test_reingestion_rebuilds_the_library_once(client, auth):
    ...
    assert [d["title"] for d in client.get("/api/documents", headers=auth).json()["items"]] == ["PTO"]
    ...
    assert [d["title"] for d in client.get("/api/documents", headers=auth).json()["items"]] == ["New"]
