...
Both the API (`backend/`) and the offline ingestion scripts (`src/`) get their
collections from here. Before this module existed there were two independent
`MongoClient` objects per process — one in `backend/db.py` and one in
`src/rag_chain.py` — each with pymongo's default pool of 100 connections.
...
`backend/db.py` binds its collection handles at module level, which triggers
...
it. If that ever changes, move the collection handles in `backend/db.py` behind
...
from config import MONGO_MAX_POOL_SIZE
